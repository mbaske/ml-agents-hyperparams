...
class Trainer(object):
    ...
    def __init__(self, sess, env, brain_name, trainer_parameters, training, summary_event):
        """
        ...
        # M.Baske - added summary_event and steps per second
    ...
    def write_summary(self, lesson_number):
        ...
        if (self.get_step % self.trainer_parameters['summary_freq'] == 0 and self.get_step != 0 and
            ...
            # M.Baske - added Trainer#
            if len(self.stats['cumulative_reward']) > 0:
                mean_reward = np.mean(self.stats['cumulative_reward'])
                logger.info("Trainer #{} {}: Step: {}. Mean Reward: {:0.3f}. Std of Reward: {:0.3f}."
                            .format(self.summary_event.trainer_num, self.brain_name, self.get_step,
                                    mean_reward, np.std(self.stats['cumulative_reward'])))
            else:
                logger.info("Trainer #{} {}: Step: {}. No episode was completed since last summary."
                            .format(self.summary_event.trainer_num, self.brain_name, self.get_step))
            # M.Baske - added event_data
            event_data = {}
            event_data['step'] = int(self.get_step)
            event_data['steps_per_second'] = (self.get_step - self.sps_step) / (time.time() - self.sps_time)
            self.sps_time = time.time()
            ...
            for key in self.stats:
                if len(self.stats[key]) > 0:
                    ...
                    event_data[key] = float(stat_mean)
                    self.stats[key] = []
            summary.value.add(tag='Info/Lesson', simple_value=lesson_number)
            summary.value.add(tag='Info/steps_per_second', simple_value=event_data['steps_per_second'])
            self.summary_writer.add_summary(summary, self.get_step)
            self.summary_writer.flush()
            self.summary_event.dispatch(data=event_data)
