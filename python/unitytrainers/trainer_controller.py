...
from unitytrainers.trainer_data import TrainerData
from unitytrainers.trainer_event import TrainerEvent


class TrainerController(object):
    # M.Baske - added config_file
    def __init__(self, env_path, run_id, save_freq, curriculum_file, fast_simulation, load, train,
        ...
        :param config_file: Path to json file containing trainer data, overriding / complementing data from yaml file
        """
        ...
        # M.Baske - added handling of trainer stats summary
        self.stop_condition_met = None
        self.trainer_data = TrainerData.from_file(config_file)  # load
        self.summary_event = TrainerEvent(self._summary_handler)
        self.summary_event.trainer_num = self.trainer_data.num

    def _summary_handler(self, summary):
        self.stop_condition_met = self.trainer_data.add_summary(summary)
    ...
    def _initialize_trainers(self, trainer_config, sess):
        ...
        for brain_name in self.env.external_brain_names:
            ...
            # M.Baske - override / complement hyperparameters
            if self.trainer_data.hyperparams:
                for k, v in self.trainer_data.hyperparams.items():
        ...
        with tf.Session() as sess:
            ...
            try:
                while any([t.get_step <= t.get_max_steps for k, t in self.trainers.items()]) or not self.train_model:
                    ...
                    # M.Baske - added conditional training stop
                    if self.stop_condition_met:
                        self.logger.info('Trainer #{0} is stopping because {1}'.format(
                            self.trainer_data.num, self.stop_condition_met))
                        break
                    if self.train_model:
                        global_step += 1
                ...
                if self.train_model:
                    ...
                    self._save_model(sess, steps=global_step, saver=saver)
                    self.trainer_data.flag_interrupted()
                pass
        self.env.close()
        if self.train_model:
            self._export_graph()
        # M.Baske - save file
        self.trainer_data.save()
