...
from unitytrainers.ppo.trainer_mod import PPOTrainer
from unitytrainers.bc.trainer_mod import BehavioralCloningTrainer
...
class TrainerController(object):
    def __enter__(self):
        ...
        self.handler_SIGINT = signal.getsignal(signal.SIGINT)
    ...
    '''
    Workaround for an issue with loggers not getting pickled using multiprocessing
    ...
    def __init__(self, env_path, run_id, save_freq, curriculum_file, fast_simulation, load, train,
        ...
        if docker_target_name == '':
            ...
            self.docker_target_name = docker_target_name
    ...
    def _set_model_path(self):
        run_id = self.run_id + self.training_data.descr
        if self.docker_training:
            self.model_path = '/{docker_target_name}/models/{run_id}'.format(
                docker_target_name=self.docker_target_name,
                run_id=run_id)
        else:
            self.model_path = './models/{run_id}'.format(run_id=run_id)
        ...
        for brain_name in self.env.external_brain_names:
            ...
            if len(self.env.external_brain_names) > 1:
                ...
                trainer_parameters['summary_path'] = '{basedir}/{name}'.format(
                    ...
                    name=str(self.run_id + self.training_data.descr) + '_' + graph_scope)
            else:
                trainer_parameters['graph_scope'] = ''
                trainer_parameters['summary_path'] = '{basedir}/{name}'.format(
                    basedir=self.summaries_dir,
                    name=str(self.run_id + self.training_data.descr))
            ...
            # complement/override loaded trainer_config
