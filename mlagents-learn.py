...
class ArgParser():
    ...
    def get_process_args(self, n: int, i: int, config_path: str) -> List[str]:
        args: List[str] = ['mlagents-learn', config_path, f'--run-id={self.get_run_id(n)}', f'--base-port={self.base_port + i}']
...
class ValueOption():
    ...
    def __str__(self) -> str:
        return f'{self.key}: {", ".join(map(str, self.values))}'
...
class Behavior():
    ...
    def __init__(self, name: str, run_id: str, config: Dict[str, Any], defaults: Dict[str, Any]):
        ...
        parsed: Dict[str, Any] = self.parse_config(config)
        ...
        if self.value_options:
            ...
            self.mod_configs.append(parsed)
...
class Config():
    ...
    :rytpe: List[List[str]]
    """
    def get_run_id_combinations(self, result: List[List[str]] = None, tmp: List[str] = None, i: int = 0) -> List[List[str]]:
...
class Runner():
                    ...
                    for i, slot in enumerate(self.slots):
                        if slot:
                            ...
                            if slot.poll() is not None:
                                ...
                                is_windows: bool = self.stop_process(i)
                                if is_windows:
                                    # Process was terminated, free up slot right away
                                    self.slots[i] = None
                                    exit = True
                                # else: Clean process shut down triggered via signal  
                                # should by caught by next poll call (not tested)
    ...
    TODO Not sure how to get mlagents subprocess to shut down
    gracefully on Windows. mlagents-learn / trainer_controller.py 
    https://github.com/Unity-Technologies/ml-agents/blob/main/ml-agents/mlagents/trainers/trainer_controller.py#L181
    doesn't handle CTRL_BREAK_EVENT, https://stackoverflow.com/a/47314406
    For now, we just terminate the process on Windows, therefore
    it won't be able to exit cleanly and store its latest model.
    This should only be an issue if we were to use stop conditions  
    for catching *successful* training runs though. If we merely 
    stop runs because they don't produce high enough rewards after
    some number of steps for instance, then we probably wouldn't 
    need their models anyway.
    ...
    def stop_process(self, i : int):
        if platform.system() is 'Windows':
            ...
            # TODO not tested yet, rather use SIGINT?
            self.slots[i].send_signal(signal.CTRL_C_EVENT)
            return False
